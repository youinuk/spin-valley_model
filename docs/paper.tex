...
three-point band over all eight registered checkpoints. Cross-seed disagreement
...
correlations differ by $0.011$ differ by $37.2$ percentage points in
...
A$_{\rm pocket}$ we set $\sigma_\lambda=\sigma_p$. The constant-velocity trajectory spans
...
$\varepsilon_v\simeq E_Z$ instead moves the crossing by $0.0014\,\mu$eV,
$0.014\%$ of the $\sigma_E=10\,\mu$eV window width, because the two
approximations displace it in the same direction and largely cancel. That
cancellation follows from $\Delta_v\ll\varepsilon_v$; at
...
$\partial_x B_z$-based kernels because it is built from $\partial_x B_x$; its
weight peaks slightly outside a proxy crossing where the
$\partial_x B_z$-based kernels peak slightly inside. This spatial offset is one
...
$\Delta\chi_\phi=\chi^{\mathrm{M2}}-\chi^{\mathrm{M1}}$ measures the effect of
...
$56.94\%$ and mean rank correlation from $0.837$ to $0.917$. The disagreement falls from $64.04$ to $43.06$ percentage points, a reduction
of $20.99$ points, or about one third. These values are retained only as an
...
$n_{\rm real}=30$ to $100$ no ansatz pair moves by more than $2.0$ points,
...
A/B$_z$, disagrees on $16.1\%$ of conditions at $n_{\rm real}=100$.
...
account for $46.4\%$ of ansatz-pair agreement. Restricting the comparison to the
...
$0.011$ in rank correlation but by $37.2$ percentage points in classification
...
\midrule
center & period$-$   & 5  & 1.0 & A$_{\rm pocket}$                 & 2/4 \\
edge   & depth$-$    & 5  & 1.0 & A, A$_{\rm pocket}$, B$_z$, B$_x$& \textbf{4/4} \\
edge   & half\_z$-$  & 5  & 1.0 & A                           & 2/4 \\
edge   & period$+$   & 5  & 1.0 & A, A$_{\rm pocket}$, B$_x$       & \textbf{4/4} \\
edge   & depth$+$    & 10 & 0.5 & A, B$_z$, B$_x$             & 3/4 \\
edge   & depth$+$    & 10 & 1.0 & A, A$_{\rm pocket}$, B$_z$, B$_x$& \textbf{4/4} \\
...
agree. High rank correlation does not guarantee classification agreement: the
A/A$_{\rm pocket}$ and A/B$_z$ pairs differ by only $0.011$ in rank
correlation but by $37.2$ percentage points in classification agreement.
...
Natural extensions include measured or finite-device micromagnet fields,
alternative noise descriptions, and microscopically derived couplings and
operator structures. Those extensions are deliberately left outside the
present comparison so that coupling-profile dependence is isolated against a
fixed geometric backdrop.
...
An optimum obtained under one phenomenological coupling remains
model-conditional. It can be strengthened either by constraining the effective
coupling independently through microscopic theory or experiment, or by showing
that the optimum persists across a justified model family. The atlas provides that cross-profile test for both the classification and the
seed-stable candidates.
